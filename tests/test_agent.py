...
class AgentTest(unittest.TestCase):
    ...
    def test_web_ui_exposes_agent_tool_trace(self) -> None:
        ...
        self.assertIn("agentTraceHtml(message.plan, message.toolResults)", APP_JS)
        self.assertIn('class="tool-trace-header"', APP_JS)
        self.assertIn('class="tool-result-summary ${hasError ? "error" : ""}"', APP_JS)
